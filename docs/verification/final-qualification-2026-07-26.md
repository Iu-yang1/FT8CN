...
FT8 p95 相对任务前为 +1.90%，FT4 为 +1.71%，均未超过 3%。曾观察到系统热/负载变化使 FT4 历史绝对值失真，因此额外在同一会话构建起始提交：25 次起始 p95 304.065 ms，当前 p95 304.085 ms，差异 +0.007%，结果哈希完全一致。原始值记录在 `performance-baseline-2026-07-26.json` 和临时机器报告中；没有减少候选、pass、round、BP/OSD 深度、LDPC 迭代、同步阈值或搜索带宽。
