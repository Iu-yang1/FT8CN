...
更新时间：2026-07-29。顶层 MIT 只覆盖 FT8CN 自有代码。APK 静态包含 GPL-3.0-or-later 的 WSJT-X core，因此组合分发不能宣称为“仅 MIT”，必须同步提供完整对应源码、许可证和修改说明。阶段 9 已通过 `scripts/check-third-party.ps1`：15 个登记组件、87 个唯一 WSJT-X 构建输入和 CycloneDX 1.5 清单一致。
